...
class ImageMatcher:
    ...
    def find_template_in_screenshot(self, template_path: str, threshold: float = 0.8, 
                                    take_new_screenshot: bool = True) -> Optional[Tuple[int, int, float]]:
        """
        在当前屏幕截图中查找模板图像
        ...
        try:
            ...
            if take_new_screenshot:
                ...
                if not screenshots:
                    ...
                    screenshot_path = os.path.join(self.screenshots_dir, max(screenshots))
            ...
            if screenshot is None or template is None:
                print(f"无法读取图像文件: screenshot={screenshot_path}, template={template_path}")
                return None
            ...
            if matches:
                ...
                print(f"最佳匹配: 位置({center_x}, {center_y}), 置信度={best_match[2]:.3f}")
                
                # 打印前5个最佳匹配
                for i, (x, y, conf) in enumerate(matches[:5]):
                    center_x_temp = x + w // 2
                    center_y_temp = y + h // 2
                    print(f"  匹配{i+1}: 位置({center_x_temp}, {center_y_temp}), 置信度={conf:.3f}")
                ...
                print(f"最高置信度位置: ({int(center_x)}, {int(center_y)}), 置信度={float(max_val):.3f}, 阈值={threshold}")
                return None
    ...
    def find_multiple_templates(self, template_paths: List[str], threshold: float = 0.8) -> List[Tuple[str, int, int, float]]:
        """
        在当前屏幕中查找多个模板图像
        ...
        for template_path in template_paths:
            match_result = self.find_template_in_screenshot(template_path, threshold, False)
            if match_result:
    ...
    def verify_element_exists(self, template_path: str, threshold: float = 0.8, 
                            timeout: int = 10) -> bool:
        """
        验证指定元素是否存在于屏幕上
    ...
    def click_template_if_found(self, template_path: str, threshold: float = 0.8) -> bool:
        """
        如果找到模板图像，则点击它
    ...
    def create_marked_screenshot_for_single_template(self, template_path: str, threshold: float = 0.8) -> Optional[str]:
        """
        ...
        try:
            ...
            if max_val >= threshold:
                ...
                # 绘制绿色圆形圈选框
                radius = max(w, h) // 2 + 15
                cv2.circle(marked_screenshot, (center_x, center_y), radius, (0, 255, 0), 4)
                ...
                # 添加"匹配成功"标签
                success_label = "匹配成功"
                label_size = cv2.getTextSize(success_label, cv2.FONT_HERSHEY_SIMPLEX, 0.8, 2)[0]
                ...
                cv2.rectangle(marked_screenshot, 
                            ...
                            (0, 255, 0), -1)
                cv2.putText(marked_screenshot, success_label, (label_x, label_y), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                
                # 添加详细信息
                info_label = f"{template_name}: {float(max_val):.3f}"
                info_y = y - 15 if y > 30 else y + h + 25
                cv2.putText(marked_screenshot, info_label, (x, info_y), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                
                print(f"模板匹配成功: {template_name}, 位置({center_x}, {center_y}), 置信度={float(max_val):.3f}")
            else:
                # 未找到匹配，标记最高置信度位置
                x, y = max_loc
                center_x = int(x + w // 2)
                ...
                # 绘制红色圆形圈选框
                radius = max(w, h) // 2 + 15
                cv2.circle(marked_screenshot, (center_x, center_y), radius, (0, 0, 255), 4)
                ...
                # 添加"匹配失败"标签
                fail_label = "匹配失败"
                label_size = cv2.getTextSize(fail_label, cv2.FONT_HERSHEY_SIMPLEX, 0.8, 2)[0]
                label_x = center_x - label_size[0] // 2
                label_y = center_y - radius - 10
                ...
                cv2.rectangle(marked_screenshot, 
                            ...
                            (label_x + label_size[0] + 5, label_y + 5),
                            (0, 0, 255), -1)
                cv2.putText(marked_screenshot, fail_label, (label_x, label_y), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                ...
                print(f"模板匹配失败: {template_name}, 最高置信度位置({center_x}, {center_y}), 置信度={float(max_val):.3f}")
            
            # 保存标记后的截图
            import time
            marked_screenshot_path = os.path.join(self.screenshots_dir, 
                                                f"marked_{template_name}_{int(time.time())}.png")
            ...
            try:
                ...
                details_text += f"匹配状态: {'成功' if max_val >= threshold else '失败'}\n"
                details_text += f"标记截图路径: {marked_screenshot_path}\n"
                
                allure.attach(details_text, name=f"📊 {template_name} 匹配详情", attachment_type=allure.attachment_type.TEXT)
    ...
    def verify_and_mark_matches(self, template_paths: List[str], threshold: float = 0.8) -> Tuple[bool, List[dict]]:
        """
        ...
        try:
            ...
            for template_path in template_paths:
                ...
                if template_matches:
                    ...
                    filtered_matches = self._apply_nms(template_matches, w, h, overlap_threshold=0.3)
                    
                    if filtered_matches:
                        all_matches.extend(filtered_matches)
                        found_any_match = True
                        
                        print(f"模板 {template_name} 找到 {len(filtered_matches)} 个匹配:")
                        ...
                        for i, (x, y, conf, _) in enumerate(filtered_matches[:3]):  # 只显示前3个
                            center_x_temp = x + w // 2
                            center_y_temp = y + h // 2
                            print(f"  匹配{i+1}: 位置({center_x_temp}, {center_y_temp}), 置信度={conf:.3f}")
                else:
                    ...
                    print(f"  最高置信度位置: ({center_x}, {center_y}), 置信度={float(max_val):.3f}")
            ...
            if all_matches:
                ...
                for i, (x, y, confidence, template_path) in enumerate(all_matches):
                    ...
                    # 不同匹配用不同颜色
                    colors = [(0, 255, 0), (0, 255, 255), (255, 0, 255), (255, 165, 0), (0, 0, 255)]
                    color = colors[i % len(colors)]
                    
                    # 绘制圆形圈选
                    cv2.circle(marked_screenshot, (center_x, center_y), radius, color, 4)
                    
                    # 绘制矩形框
                    cv2.rectangle(marked_screenshot, (x, y), (x + w, y + h), color, 2)
                    
                    # 添加序号标签
                    label_bg_size = cv2.getTextSize(f"{i+1}", cv2.FONT_HERSHEY_SIMPLEX, 0.8, 2)[0]
                    cv2.rectangle(marked_screenshot, 
                                (center_x - label_bg_size[0]//2 - 5, center_y - label_bg_size[1]//2 - 5),
                                (center_x + label_bg_size[0]//2 + 5, center_y + label_bg_size[1]//2 + 5),
                                color, -1)
                    cv2.putText(marked_screenshot, f"{i+1}", 
                              (center_x - label_bg_size[0]//2, center_y + label_bg_size[1]//2), 
                              cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                    
                    # 添加详细信息标签
                    info_label = f"{template_name}: {confidence:.3f}"
                    label_y = y - 15 if y > 30 else y + h + 25
                    cv2.putText(marked_screenshot, info_label, (x, label_y), 
                              cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
                
                # 保存圈选标记后的截图
                import time
                marked_screenshot_path = os.path.join(self.screenshots_dir, 
                                                    f"marked_matches_{int(time.time())}.png")
                cv2.imwrite(marked_screenshot_path, marked_screenshot)
                ...
                try:
                    ...
                    allure.attach(details_text, name="匹配区域详细信息", attachment_type=allure.attachment_type.TEXT)
