...
# 设置脚本执行权限
echo "🔐 设置脚本执行权限..."
chmod +x "${PROJECT_ROOT}/scripts/run/run_tests.sh" || true
chmod +x "${PROJECT_ROOT}/scripts/run/run_single_test.sh" || true
chmod +x "${PROJECT_ROOT}/scripts/report/stop_report_server.sh" || true
chmod +x "${PROJECT_ROOT}/scripts/report/cleanup_report_servers.sh" || true
chmod +x "${PROJECT_ROOT}/scripts/setup.sh" || true
...
echo "   3. 运行测试: ./scripts/run/run_tests.sh"
