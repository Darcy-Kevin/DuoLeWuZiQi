...
def generate_common_template_from_coordinates(params):
    ...
    try:
        ...
        print("✅ 设备连接成功!")
        print(
            f"📱 设备信息: {driver.info.get('brand', 'Unknown')} {driver.info.get('model', 'Unknown')}"
        )
        ...
        if params["x"] >= width_img or params["y"] >= height_img:
            print(
                f"❌ 指定位置({params['x']}, {params['y']})超出截图范围({width_img}x{height_img})"
            )
            return None
        ...
        common_template_path = os.path.join(
            common_template_dir, f'{params["template_name"]}_common.png'
        )
        ...
        cv2.rectangle(
            marked_screenshot, (start_x, start_y), (end_x, end_y), (0, 255, 0), 3
        )
        cv2.putText(
            marked_screenshot,
            f"Common Template: ({start_x}, {start_y})",
            (start_x, start_y - 10),
        ...
        # 检查是否在预期位置附近
        distance = np.sqrt(
            (max_loc[0] - params["x"]) ** 2 + (max_loc[1] - params["y"]) ** 2
        )
